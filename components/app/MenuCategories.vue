<template>
  <div class="flex flex-col">
    <NuxtLink :class="[sizeClass, colorClass]" v-for="i in short" :to="`/api/${i.category_name}`">
      {{ i.category_name }}
    </NuxtLink>
...
const { data: list } = useAsyncData('topCategories', async () => {
...
const short = computed(() => {
  ...
  return list.value.filter((i) => i.category_name.length < 20)
})
</script>
